...
function requireGemini(_req: AuthRequest, res: Response, next: NextFunction): void {
...
secondBrainRouter.post('/capture', requireGemini, async (req: AuthRequest, res): Promise<void> => {
  try {
    ...
    fetch('http://127.0.0.1:7786/ingest/92ad1406-29dc-4f4f-ac56-debf92fe1219', {
      ...
      body: JSON.stringify({
        sessionId: '3b680a',
        location: 'secondBrain.ts:capture',
        message: 'Capture route entered',
        data: { rawTextLen: rawText.length, hasRawText: !!rawText?.trim() },
        hypothesisId: 'H1',
        timestamp: Date.now(),
      }),
    }).catch(() => {});
    ...
    const result = await captureThought(sanitizeFreeText(rawText, 10000), 'web');
    // #region agent log
    fetch('http://127.0.0.1:7786/ingest/92ad1406-29dc-4f4f-ac56-debf92fe1219', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Debug-Session-Id': '3b680a' },
      body: JSON.stringify({
        sessionId: '3b680a',
        location: 'secondBrain.ts:capture',
        message: 'Capture route success',
        data: { thoughtId: result.thoughtId, category: result.category, routed: result.routed },
        hypothesisId: 'H1',
        timestamp: Date.now(),
      }),
    }).catch(() => {});
    ...
    // #endregion
    const message = err instanceof Error ? err.message : 'Failed to capture thought';
...
secondBrainRouter.post('/search', requireGemini, async (req: AuthRequest, res): Promise<void> => {
  try {
    ...
    try {
      results = await semanticSearch(sanitizeFreeText(query, 2000), threshold, limit);
    } catch (err) {
...
secondBrainRouter.post('/query', requireGemini, async (req: AuthRequest, res): Promise<void> => {
  try {
    ...
    const answer = await queryWithRag(sanitizeFreeText(question, 2000), 5);
    res.json({ answer });
  } catch (err) {
...
secondBrainRouter.get('/people', async (req, res) => {
  try {
    ...
    const message = err instanceof Error ? err.message : 'Failed to list people';
    res.status(500).json({ error: message });
  }
});
...
secondBrainRouter.get('/admin', async (req, res) => {
  try {
    ...
    const message = err instanceof Error ? err.message : 'Failed to list admin tasks';
    res.status(500).json({ error: message });
  }
});
...
secondBrainRouter.get('/thoughts', async (req, res) => {
  try {
    ...
    const message = err instanceof Error ? err.message : 'Failed to list thoughts';
    res.status(500).json({ error: message });
  }
});
...
secondBrainRouter.patch('/projects/:id', async (req: AuthRequest, res): Promise<void> => {
  try {
    ...
    const status =
      typeof req.body?.status === 'string' && ['active', 'paused', 'done'].includes(req.body.status)
        ? req.body.status
        : undefined;
    const name = typeof req.body?.name === 'string' ? req.body.name : undefined;
    const goal = typeof req.body?.goal === 'string' ? req.body.goal : undefined;
    const next_action =
      typeof req.body?.next_action === 'string' ? req.body.next_action : undefined;
    const due_date =
      req.body?.due_date === null || typeof req.body?.due_date === 'string'
        ? req.body.due_date
        : undefined;
    const area =
      req.body?.area === null || typeof req.body?.area === 'string' ? req.body.area : undefined;
    const notes =
      req.body?.notes === null || typeof req.body?.notes === 'string' ? req.body.notes : undefined;
    if (
      status === undefined &&
      name === undefined &&
      goal === undefined &&
      ...
      res.status(400).json({
        error: 'At least one of status, name, goal, next_action, due_date, area, notes is required',
      });
      return;
    }
...
secondBrainRouter.patch('/admin/:id', async (req: AuthRequest, res): Promise<void> => {
  try {
    const id = parseId(req);
    if (!id) {
    ...
    const status =
      typeof req.body?.status === 'string' && ['pending', 'done'].includes(req.body.status)
        ? req.body.status
        : undefined;
...
secondBrainRouter.patch('/ideas/:id', async (req: AuthRequest, res): Promise<void> => {
  try {
    const id = parseId(req);
    if (!id) {
    ...
    const body =
      req.body?.body === null || typeof req.body?.body === 'string' ? req.body.body : undefined;
    const area =
      req.body?.area === null || typeof req.body?.area === 'string' ? req.body.area : undefined;
    if (status === undefined && title === undefined && body === undefined && area === undefined) {
      res.status(400).json({
...
secondBrainRouter.patch('/thoughts/:id', async (req: AuthRequest, res): Promise<void> => {
  try {
    ...
    const rawText =
      typeof req.body?.rawText === 'string' ? sanitizeFreeText(req.body.rawText, 10000) : undefined;
    const category =
    ...
    if (rawText === undefined && category === undefined && confidence === undefined) {
      res
        .status(400)
        .json({ error: 'At least one of rawText, category, or confidence is required' });
      return;
    }
...
secondBrainRouter.post(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to route thought';
      res.status(500).json({ error: message });
    }
  },
);
...
secondBrainRouter.delete('/thoughts/:id', async (req: AuthRequest, res): Promise<void> => {
  try {
    const id = parseThoughtId(req);
    if (!id) {
      res.status(400).json({ error: 'Invalid thought ID' });
...
secondBrainRouter.get('/digest', async (_req, res): Promise<void> => {
  try {
    ...
    const message = err instanceof Error ? err.message : 'Failed to get digest data';
    res.status(500).json({ error: message });
  }
});

/** GET /api/second-brain/digest/generate — AI-generated morning digest */
secondBrainRouter.get('/digest/generate', requireGemini, async (_req, res): Promise<void> => {
  try {
...
secondBrainRouter.get('/review', async (_req, res): Promise<void> => {
  try {
    ...
    const message = err instanceof Error ? err.message : 'Failed to get review data';
    res.status(500).json({ error: message });
  }
});

/** GET /api/second-brain/review/generate — AI-generated weekly review */
secondBrainRouter.get('/review/generate', requireGemini, async (_req, res): Promise<void> => {
  try {
...
secondBrainRouter.post(
  ...
  async (_req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to backfill embeddings';
