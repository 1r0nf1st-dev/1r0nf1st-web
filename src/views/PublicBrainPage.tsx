      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <aside className="lg:w-80 flex-shrink-0 space-y-4">
          ...
          {nodes.length === 0 ? (
            ...
            <p className="text-sm text-muted">Switch to List to browse, or use the graph on the right.</p>
          )}
        </aside>
        ...
        <section className="flex-1 min-w-0 space-y-4">
          ...
          {selectedNode ? (
            <NodeDetailReadOnly
              node={selectedNode}
              currentUserId={currentUserIdForReactions}
            />
          ) : (
            <div className={cardClasses}>
          ...
          {user?.id && (
            <ChatPanel brainOwnerId={profile.id} />
          )}
        </section>
      </div>
    </div>
