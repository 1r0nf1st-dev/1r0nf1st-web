...
export const NotesListItem = memo(
  ({ note, isSelected, onClick }: NotesListItemProps): JSX.Element => {
    const preview = note.content_text
      ? note.content_text.substring(0, PREVIEW_LENGTH_LIST) +
        (note.content_text.length > PREVIEW_LENGTH_LIST ? '...' : '')
      : '';
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {preview && <div className="text-sm text-muted line-clamp-2 mb-2">{preview}</div>}
  ...
  (prevProps, nextProps) => {
    return prevProps.note.id === nextProps.note.id && prevProps.isSelected === nextProps.isSelected;
  },
);
